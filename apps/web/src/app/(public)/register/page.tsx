    ...
    const handleGoogleCredentialResponse = async (response: any) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const data = await res.json();
                showError(data.message || data.error || 'Google registration failed');
            }
        } catch (error) {
            ...
            <div className="w-full max-w-md bg-white border border-slate-200 rounded-[32px] p-8 md:p-12 shadow-2xl shadow-slate-200/50 relative">
                ...
                <form onSubmit={handleSubmit} className="animate-fadeUp">
                    ...
                    <div className="text-center">
                        ...
                        <Link href="/login" className="text-[14px] font-medium text-slate-900 hover:text-emerald-500 transition-colors">Log in</Link>
                    </div>
                </form>
            </div>
        </main>
