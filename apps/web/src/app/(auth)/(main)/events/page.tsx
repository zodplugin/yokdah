    ...
    if (!src || error) {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-[var(--bg2)] to-[var(--bg3)] relative overflow-hidden">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-2 opacity-50">{category || 'Event'}</span>
            </div>
        );
    }
...
export default function EventsPage() {
    ...
    const isHot = (event: any) => {
        return (event.lookingCount || 0) > 10;
    };
    ...
    return (
        <div className="p-6 md:p-12 max-w-6xl mx-auto min-h-screen">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div>
                    <h1 className="font-sans font-extrabold tracking-tight text-[clamp(36px,5vw,48px)] leading-[1.1] mb-2 tracking-[-0.02em]">Discover <em>events</em></h1>
                    <p className="text-[15px] text-slate-500">Find your people before the event even starts.</p>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="relative w-full md:w-[280px]">
                        ...
                        <input
                            ...
                            className="w-full bg-white border border-slate-200 rounded-[100px] pl-10 pr-4 py-2.5 text-[14px] outline-none focus:border-[#059669] focus:ring-2 focus:ring-emerald-500/20 transition-all"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-[24px] p-4 md:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 shadow-md shadow-slate-100">
                <div className="flex items-center gap-3.5">
                    <div className="w-10 h-10 flex-shrink-0 rounded-full bg-slate-50 flex items-center justify-center text-emerald-500">
                        <MapPin size={18} />
                    </div>
                    <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-10 pb-6 border-b border-slate-200">
                <div className="relative">
                    ...
                    {showCategoryDropdown && (
                        <div className="absolute z-50 top-full mt-1 bg-white border border-slate-200 rounded-[20px] shadow-2xl shadow-slate-200/50 backdrop-blur-xl bg-white/95 overflow-hidden min-w-[160px]">
                            {CATEGORIES.map((cat) => (
                                <button
                                    key={cat.value}
                                    onClick={() => handleCategorySelect(cat.value)}
                                    className={`w-full text-left px-4 py-2.5 text-[13px] font-medium hover:bg-slate-50 transition-colors ${selectedCategory === cat.value ? 'bg-emerald-50 text-emerald-600' : 'text-slate-500'}`}
                                >
                                    {cat.name}
                                </button>
                            ))}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-[100px] text-[13px] font-medium transition-all ${!selectedCategory ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:-translate-y-0.5 transition-transform' : 'bg-white border border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-50 hover:border-slate-300'}`}
                    >
                        All
                    </button>
                    <button
                        onClick={() => handleCategorySelect("concert")}
                        className={`flex items-center gap-2 px-4 py-2 rounded-[100px] text-[13px] font-medium transition-all ${selectedCategory === "concert" ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:-translate-y-0.5 transition-transform' : 'bg-white border border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-50 hover:border-slate-300'}`}
                    >
                        Concerts
                    </button>
                    <button
                        onClick={() => handleCategorySelect("festival")}
                        className={`flex items-center gap-2 px-4 py-2 rounded-[100px] text-[13px] font-medium transition-all ${selectedCategory === "festival" ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:-translate-y-0.5 transition-transform' : 'bg-white border border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-50 hover:border-slate-300'}`}
                    >
                        Festivals
                    </button>
                    <button
                        onClick={() => handleCategorySelect("activity")}
                        className={`flex items-center gap-2 px-4 py-2 rounded-[100px] text-[13px] font-medium transition-all ${selectedCategory === "activity" ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:-translate-y-0.5 transition-transform' : 'bg-white border border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-50 hover:border-slate-300'}`}
                    >
                        Activities
                    </button>
                    <button
                        onClick={() => handleCategorySelect("sport")}
                        className={`flex items-center gap-2 px-4 py-2 rounded-[100px] text-[13px] font-medium transition-all ${selectedCategory === "sport" ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:-translate-y-0.5 transition-transform' : 'bg-white border border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-50 hover:border-slate-300'}`}
                    >
                        Sports
                    </button>
                </div>
            ...
            {loading ? (
                <div className="flex items-center justify-center py-20">
                    <div className="text-slate-400 text-[15px]">Loading events...</div>
                </div>
            ) : events.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    <MapPin size={48} className="text-slate-400 mb-4" />
                    <h3 className="text-[24px] font-medium mb-2 text-slate-900">No events found</h3>
                    <p className="text-[15px] text-slate-500">Try changing your location or category filter</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {events.map((ev) => (
                        <Link href={`/events/${ev.id}`} key={ev.id} className="group flex flex-col bg-white border border-slate-200 rounded-[24px] overflow-hidden hover:border-slate-300 transition-all hover:shadow-xl shadow-slate-200/50 hover:-translate-y-1">
                            <div className="h-44 flex items-center justify-center relative overflow-hidden bg-slate-50 border-b border-slate-200">
                                <EventImage src={ev.coverImage} alt={ev.name} category={ev.category} />
                                <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                            </div>
                            <div className="p-5 flex-1 flex flex-col">
                                <h3 className="font-sans font-extrabold tracking-tight text-[22px] leading-[1.2] mb-3 group-hover:text-emerald-600 transition-colors">{ev.name.split('—')[0]}</h3>
                                <div className="flex items-center justify-between mb-4">
                                    <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-slate-400 bg-slate-50 px-2.5 py-1 rounded-[4px] border border-slate-200">{ev.category}</span>
                                ...
                                <div className="space-y-2.5 mb-6">
                                    <div className="flex items-center gap-2.5 text-[13px] text-slate-500 font-medium">
                                        <Calendar size={16} className="text-slate-400" /> {formatDate(ev.date)}
                                    </div>
                                    <div className="flex items-center gap-2.5 text-[13px] text-slate-500 font-medium">
                                        <MapPin size={16} className="text-slate-400" /> {ev.venue}
                                    </div>
                                </div>
            ...
            {pagination && pagination.pages > 1 && (
                <div className="flex items-center justify-center gap-4 mt-12 pb-12">
                    <button
                        ...
                        className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center disabled:opacity-30 hover:bg-slate-50 transition-colors"
                    >
