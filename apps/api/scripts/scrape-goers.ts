...
const BASE_URL = 'https://www.goersapp.com'
const LIST_URL = 'https://www.goersapp.com/events'
...
function mapAnyToEvent(candidate: AnyObj) {
  ...
  return {
    ...
    description: `Experience ${name} at ${venue}. Tickets available on Goers.`,
    coverImage,
...
async function scrapeGoers() {
  try {
    console.log('Connecting to MongoDB...')
    await mongoose.connect(MONGODB_URI)
    console.log('Connected successfully.')

    console.log(`Fetching Goers page: ${LIST_URL}`)
    const html = await fetchText(LIST_URL, { headers: { Referer: BASE_URL } })

    const nextData = extractNextData(html)
    if (!nextData) {
      throw new Error('Goers: could not find __NEXT_DATA__. Site may block bots or use a different renderer.')
    }

    const arrays = findLikelyEventArrays(nextData)
    if (arrays.length === 0) {
      throw new Error('Goers: could not locate any likely event arrays in __NEXT_DATA__. Mapping needs updates.')
    }
    ...
    for (const ev of mapped) {
      try {
        await Event.findOneAndUpdate(
          { source: 'goers', sourceId: ev.sourceId },
          ev,
          { upsert: true, new: true }
        )
        importedCount++
      } catch (err) {
        console.error(`Error importing Goers event ${ev.name}:`, err)
        skippedCount++
      }
    }

    console.log('--- Goers Import Summary ---')
    ...
    console.error('Scraper error (Goers):', error)
    process.exit(1)
  }
}
